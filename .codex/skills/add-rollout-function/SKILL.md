...
description: Guide for adding a new rollout function in slime and wiring it through --rollout-function-path. Use when you need custom rollout data generation logic, custom train/eval rollout outputs, or a migration away from the default sglang rollout path.
---

# Add Rollout Function

Source: `.claude/skills/add-rollout-function/SKILL.md`

Relevant rules to consult when available:
- `.codex/rules/distributed.md`
- `.codex/rules/testing.md`

Use this skill in Codex when implementing a custom rollout function and integrating it safely with the slime training and eval flow.

## When to Use

Use this skill when:

- You need to add a new rollout task or rollout generation function
- You need to replace the default `slime.rollout.sglang_rollout.generate_rollout`
- You need to customize train or eval data generation behavior
...
If the task needs engine-based async generation and rewards, use the sglang path as your base.
If the task is file or buffer driven and simple, use the sft path as your base.
...
- `tokens`
- `response_length`
- `reward` (or a reward dict if your setup uses keyed rewards)
- `status`
...
- Returning raw Python lists or dicts with mismatched schema in the custom path
- Implementing only the training branch and forgetting the evaluation branch
- Generating samples without required fields (`tokens`, `response_length`, `reward`, `status`)
- Using blocking-heavy logic in high-frequency rollout paths without batching or concurrency control
...
- Wiring and loading: `slime/ray/rollout.py`
